...
return (
    ...
    <h2>{title}</h2>
      ...
      <button disabled={isVoteLoading} onClick={() => handleVote(1)}>Upvote</button>
       <button disabled={isVoteLoading} onClick={() => handleVote(-1)}>Downvote</button>
       {votesCountError && <p className="error">{votesCountError}</p>}
       {votesSuccessMessage && <p className="success">{votesSuccessMessage}</p>}
